...
class AlignmentMetrics:
    ...
    def cka(feats_A, feats_B, kernel_metric='ip', rbf_sigma=1.0, unbiased=False):
        """Computes the Centered Kernel Alignment (CKA) between features."""
        
        # S'assurer que les tenseurs sont sur le même device et de même type
        if feats_A.device != feats_B.device:
            feats_B = feats_B.to(feats_A.device)
        
        if feats_A.dtype != feats_B.dtype:
            feats_B = feats_B.to(feats_A.dtype)
        
        # Vérifier qu'on a le même nombre d'échantillons
        assert feats_A.shape[0] == feats_B.shape[0], f"Nombre d'échantillons différent: {feats_A.shape[0]} vs {feats_B.shape[0]}"
        ...
        # Compute CKA - amélioration de la stabilité numérique
        ...
        # Clamp pour éviter les valeurs numériquement instables
        cka_value = torch.clamp(cka_value, -1.0, 1.0)
    ...
    def edit_distance_knn(feats_A, feats_B, topk):
        ...
        edit_distance = compute_distance(knn_A, knn_B, EditDistance())
...
def hsic_unbiased(K, L):
    ...
    # Compute HSIC using the corrected formula
    # Term 1: trace(K_tilde @ L_tilde)
    term1 = torch.trace(K_tilde @ L_tilde)
    
    # Term 2: sum(K_tilde) * sum(L_tilde) / ((m-1)(m-2))
    term2 = torch.sum(K_tilde) * torch.sum(L_tilde) / ((m - 1) * (m - 2))
    
    # Term 3: -2 * sum(K_tilde @ ones) * sum(L_tilde @ ones) / (m-2)
    # où sum(K_tilde @ ones) = sum over columns of K_tilde
    k_col_sums = torch.sum(K_tilde, dim=0)
    l_col_sums = torch.sum(L_tilde, dim=0)
    term3 = -2 * torch.sum(k_col_sums * l_col_sums) / (m - 2)

    HSIC_value = (term1 + term2 + term3) / (m * (m - 3))
    
    return HSIC_value


def hsic_biased(K, L):
    """ Compute the biased HSIC (the original CKA) avec centrage correct """
    n = K.shape[0]
    
    # Matrice de centrage H = I - (1/n) * 1 * 1^T
    H = torch.eye(n, dtype=K.dtype, device=K.device) - torch.ones(n, n, dtype=K.dtype, device=K.device) / n
    
    # HSIC(K,L) = trace(HKHL) = trace(K @ H @ L @ H)
    # Mais il est plus efficace de calculer: trace((HK) @ (HL))
    HK = H @ K
    HL = H @ L
    
    return torch.trace(HK @ HL)
